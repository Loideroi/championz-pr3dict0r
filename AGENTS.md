...
`npm run typecheck`, `npm run lint` (app tree **and** `relayer/`; budgets are errors; exceptions need `expires YYYY-MM-DD`, inline `/* eslint … */` config comments are forbidden), `npm run arch` (dependency-cruiser layering for the app tree **and** `relayer/` — see `ARCHITECTURE.md`), `npm run deadcode:ci` (knip: files, dependencies, unlisted, unresolved; workspaces `.` and `relayer`), `npm run dup` (jscpd ≤ 1%), `npm test`, `npm run check:i18n`; in `relayer/`: `npm test` (incl. the scripts → dist import check); in `contracts/`: `npm run slither` (blocking on new or moved High/Medium findings; triage in `slither-triage.json`). Baselines, exceptions and tier floors: `docs/REVIEW_TIERS.md`. Guardrail-complete since 2026-09-11 (Loideroi LLM Wiki, Development Lifecycle → guardrail stack).
