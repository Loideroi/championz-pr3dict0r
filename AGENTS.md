...
- **Pushes without asking** cover only already-committed work on `feat/*`, `fix/*`, `docs/*`, `chore/*`, `refactor/*`, `perf/*` branches (the settings encode exactly these). Anything on `main`, deploys (`vercel`), dependency installs, and force-pushes are ask-first or denied.
