/**
 * Static ISO 3166-1 alpha-2 country list (English names, generated from CLDR
 * region display names). Shared by the sign-up form (searchable select with
 * flag emoji) and the profile API (server-side country validation).
 *
 * Flags are derived from the code (regional-indicator codepoints) — no emoji
 * stored, so the list stays byte-cheap and SSR-deterministic.
 */
...
/** True when `code` is a known ISO 3166-1 alpha-2 code from our list. */
export function isValidCountryCode(code: string): boolean {
  return CODES.has(code);
}

/** "NL" → 🇳🇱 via Unicode regional-indicator symbols. Pure and SSR-safe. */
