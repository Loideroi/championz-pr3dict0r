...
| Lint exceptions (`scripts/check-lint-exceptions.mjs`, ESLint-API driven over the whole linted tree, tested in `scripts/check-lint-exceptions.test.ts`) | 5, all `expires 2026-10-31` | Blocking: a suppression directive without a real `expires YYYY-MM-DD` date, past it, or more than 180 days out fails the build anywhere ESLint lints; inline config comments (`/* eslint rule: off */`) are forbidden outright because they bypass suppression tracking; `reportUnusedDisableDirectives: error` fails a stale directive once the function is fixed |
