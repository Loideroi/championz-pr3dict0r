...
| dependency-cruiser (`npm run arch`: app, components, hooks, lib, i18n, content, middleware.ts **and, since 2026-09-13, relayer/src, test, scripts, vendor**) | 0 violations over 410 dependencies / 177 modules locally, 174 in the CI `app` job (`relayer/node_modules` absent there, so viem/zod/vitest resolve once from the root — same graph; app 123 / 297 on 2026-09-11; relayer 54 / 111 on 2026-09-13 — 0 cycles, the two orphans being the exempted `scripts/bot-poll.mjs` and the vendored types; identical with or without a local `relayer/dist`) | Blocking: no cycles, no upward imports, no orphans outside framework entry files / relayer scripts / the vendored types, no runtime devDependency imports (relayer judged against `relayer/package.json`), no relayer import of a root-only package, no import between the two trees except `lib/admin/pipeline.lockstep.test.ts`, relayer `test/` imported by nothing, `vendor/` importing nothing (`ARCHITECTURE.md` → relayer layers) |
| knip (`npm run deadcode:ci`: files, dependencies, unlisted, unresolved; two workspaces `.` and `relayer` since 2026-09-13) | 0 / 0 / 0 / 0 (informational via `npm run deadcode`: app 19 unused exports + 7 unused types; relayer 25 unused exports + 10 unused types + 1 duplicate export) | Blocking on the four high-signal categories (`unresolved` added 2026-09-13 at a measured 0 in both workspaces); exports/types are slow-burn, not gated. Known blind spots and where the gate lives instead: a root-listed package imported from `relayer/` (→ dependency-cruiser `relayer-no-unlisted-package`), an unresolved import into gitignored `relayer/dist` (→ `relayer/test/scripts-dist-imports.test.ts`, required `relayer` check) |
...
- ~~**Relayer dependency-cruiser + knip**~~ **Done 2026-09-13** (PR #109): `relayer/src`, `test`, `scripts`, `vendor` cruised from the root config (relayer layering rules, entry exemptions listed with reasons, `relayer/dist` excluded) and a `relayer` knip workspace (scripts and the vendored types as entries, dist mapped back to src; `unresolved` added to the CI set); zero new dependencies, both run in the existing `app` steps (the `relayer` job is Node 22 / npm 10 and cannot install the root lockfile without a second install, and neither tool needs `relayer/node_modules` — verified with it absent). Two blind spots found while proving the gate red-capable are covered elsewhere and named in the table row: root-only packages (dependency-cruiser rule) and misspelled dist imports (`relayer/test/scripts-dist-imports.test.ts`).
