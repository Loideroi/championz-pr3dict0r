...
Guardrail-complete since 2026-09-11 (app tree) and 2026-09-12 (contracts slither): every gate below fails CI inside a required check (strict, enforce-admins) — the `app` check unless the row says otherwise. Wired warn-only 2026-08-27; measured again 2026-09-11 before the flip.
...
- ~~**Contracts security scanning**~~ **Done 2026-09-12** (PR C): slither 0.11.4 in the `contracts` required check, blocking on new or stale High/Medium findings via `scripts/check-slither.mjs` + `slither-triage.json`. Not added: solhint (no config exists; a new tool) and Foundry (no Foundry project here).
