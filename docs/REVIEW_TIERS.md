...
- ~~**CI npm bootstrap**~~ **Done 2026-09-13** (PR D): the `app` job runs on Node 24 (bundled npm 11, asserted by a `grep` step) and the pre-guard `npm install -g npm@11` is gone. Still floating within Node 24 / npm 11 patch releases (setup-node major pin only) — that is a toolchain pin, not a supply-chain guard; the lockfile-lint + `npm ci` guard is unchanged. Other jobs stay on Node 22 (separate lockfiles that install under npm 10; the relayer's `engines` is `>=22`; production workflows untouched).
