...
| slither 0.11.4 (`contracts/`, `npm run slither` there; **required check `contracts`**) | 43 findings on 2026-09-12: 1 High + 7 Medium, all triaged in `slither-triage.json` with the exact expression and reason (SECURITY_FINDINGS.md §Slither); 35 Low/Informational, reported only | Blocking on any **new** High/Medium finding and on any **stale** triage entry (`scripts/check-slither.mjs`). Triage ids hash the finding's source mapping, so an edit that moves those lines re-surfaces the findings: re-triage them consciously in the same PR (remove the stale entries, add the new ids with reasons) — the database can only shrink or be deliberately refreshed. Slither severity and project disposition differ: the 7 Mediums are "reviewed informational" in SECURITY_FINDINGS but stay Medium here so a new Medium blocks |
