...
| Lint exceptions (`scripts/check-lint-exceptions.mjs`, whole linted tree, tested in `scripts/check-lint-exceptions.test.ts`) | 5, all `expires 2026-10-31` | Blocking: an `eslint-disable` directive without a real `expires YYYY-MM-DD` date, or past it, fails the build anywhere ESLint lints; `reportUnusedDisableDirectives: error` fails a stale one once the function is fixed |
