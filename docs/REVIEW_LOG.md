...
Entries dated 2026-09-12 or later are linted in CI by `scripts/lint-review-log.mjs` (a byte-identical copy of the Loideroi LLM Wiki's judge, adopted from the BMAD Method assessment): every entry must carry a tier (highest mentioned wins), at least one exact model id, a labeled verdict, a complete tally (`Tally.` N Blocker / N Major / N Minor / N Nit, or a Sev table with its separator row), and `Checked.` and `Dismissed.` fields with content (dismissals never silent; `none` is content); Tier 2 and 3 entries also carry `verification-gap.` (would any test fail if this behavior broke?), `named-set.` (partial special-casing of a fixed set), and `Missing.` (what the reviewer looked for and did not find) as labeled fields. Headings carrying `ESCAPE AUDIT`, `OWNER WAIVER`, or `EXCEPTION RECORD` are contract-defined records, not reviews, and are exempt. Run `npm run check:review-log` before pushing.
